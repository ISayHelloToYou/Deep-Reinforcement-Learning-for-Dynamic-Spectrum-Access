...
NUM_USERS = 3  # type: int # Total number of users
NUM_SIZE = 3
...
noise = 0.1
step_size = 3  # length of history sequence for each datapoint  in batch
state_size = NUM_CHANNELS + 1 + NUM_SIZE + NUM_CHANNELS + 1# length of input (2 * k + 2)   :k = NUM_CHANNELS
action_size = (NUM_CHANNELS + 1)* NUM_SIZE # length of output  (k+1)
...
env = env_network(NUM_USERS, NUM_CHANNELS, NUM_SIZE , ATTEMPT_PROB, TABLE)
...
for time_step in range(TIME_SLOTS):
    ...
    # print(actions[:,-1], actions[:,-1].shape)
    # print(tf.one_hot(actions[:,-1], 5))
