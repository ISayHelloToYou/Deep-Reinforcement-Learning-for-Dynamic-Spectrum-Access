...
class QNetwork:
    def __init__(self, learning_rate=0.01, state_size=4, 
        ...
        with tf.variable_scope(name):
            ...
            self.w2 = tf.Variable(tf.random_uniform([hidden_size,hidden_size]), dtype = tf.float32, name = 'weights2')
            self.b2 = tf.Variable(tf.constant(0.1,shape=[hidden_size]), dtype = tf.float32, name = 'biases2')
            self.h2 = tf.matmul(self.reduced_out,self.w2) + self.b2
            self.h2 = tf.nn.relu(self.h2)
            self.h2 = tf.contrib.layers.layer_norm(self.h2)

            self.w3 = tf.Variable(tf.random_uniform([hidden_size, hidden_size]), dtype=tf.float32, name='weights3')
            self.b3 = tf.Variable(tf.constant(0.1, shape=[hidden_size]), dtype=tf.float32, name='biases3')
            self.h3 = tf.matmul(self.h2,self.w3) + self.b3
            self.h3 = tf.nn.relu(self.h3)
            self.h3 = tf.contrib.layers.layer_norm(self.h3)

            self.w4 = tf.Variable(tf.random_uniform([hidden_size,action_size]), dtype = tf.float32, name = 'weights4')
            self.b4 = tf.Variable(tf.constant(0.1,shape=[action_size]), dtype = tf.float32, name = 'biases4')
            self.output = tf.matmul(self.h3,self.w4, name = 'output') + self.b4
